...
class DatabasesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create database" do
    assert_difference('Database.count') do
      post databases_url, params: { database: { author: @database.author, date: @database.date, description: @database.description, name: @database.name } }
    end
  ...
  test "should update database" do
    patch database_url(@database), params: { database: { author: @database.author, date: @database.date, description: @database.description, name: @database.name } }
